...
func makeTitle() drawable.Unit {
	return drain.UnitFromLines(
		*line.New("Etiam dictum gravida"),
		*line.New("=", spec.Cover()),
	)
}

func makeKernel() drawable.Unit {
	hstack := stack.NewHStack()

	ascii := lines.UnitFromLines(
		*line.New(`  .·:·.                `),
		*line.New(`  :   :          /\    `),
		*line.New(`  '·.·'    /\   /  \   `),
		*line.New(`          /  \ /    \  `),
